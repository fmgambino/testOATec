...
async function exportSelectedPdf(sections) {
  ...
  const logoUrl = "https://www.oatec.org.ar/wp-content/uploads/2022/12/logo-oatec-con-blanco-para-menu-web-2017-e1671590002953.png";
  const logoData = await imageUrlToDataUrl(logoUrl);
  if (logoData) {
    try { doc.addImage(logoData, "PNG", 14, 10, 22, 22); } catch (_) {}
  }
  doc.setFontSize(18);
  doc.setTextColor(20, 35, 70);
  doc.text("Instituto San Miguel · OATec ITBA 2026", 42, 16);
  doc.setFontSize(13);
  doc.text("Reporte General", 42, 24);
  doc.setFontSize(9);
  doc.setTextColor(80, 90, 110);
  doc.text(`Fecha y hora: ${new Date().toLocaleString("es-AR")}`, 42, 31);
  doc.text(`Generado desde el panel administrador`, 42, 36);
...
document.getElementById("adminLogoutBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    await bootstrap(true);
    showView("home");
    await showAlert({ icon: "success", title: "Sesión cerrada", text: "Saliste del panel administrador correctamente." });
  } catch (error) {
    console.error(error);
