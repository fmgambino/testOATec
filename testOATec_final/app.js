...
const ADMIN_USER = APP_CONFIG.adminUser || "admin";
const ADMIN_PASS = APP_CONFIG.adminPass || "Admin0381$$";
...
async function renderAdmin() {
  ...
  el.adminStatusNote.textContent = APP_CONFIG.mode === "superdb" ? "Modo SuperDB" : "Modo local";
...
const dataLayer = (() => {
  if (APP_CONFIG.mode === "superdb" && APP_CONFIG.url && APP_CONFIG.anonKey && window.supabase) {
    const client = window.supabase.createClient(APP_CONFIG.url, APP_CONFIG.anonKey);
    return {
      async loadAll() {
        const [{ data: tests }, { data: questions }, { data: attempts }] = await Promise.all([
          client.from("tests").select("*").order("created_at", { ascending: false }),
          client.from("questions").select("*").order("position", { ascending: true }),
          client.from("attempts").select("*").order("created_at", { ascending: false })
        ]);
        const merged = (tests || []).map((test) => ({
          ...test,
          questions: (questions || []).filter((q) => q.test_id === test.id)
        }));
        return { tests: normalizeLoadedTests(merged), attempts: attempts || [] };
      },
      async saveAttempt(payload) {
        await client.from("attempts").insert(payload);
      },
      async createTest(test, questions) {
        const { data: inserted, error } = await client.from("tests").insert(test).select().single();
        if (error) throw error;
        const withTest = questions.map((q, index) => ({ ...q, test_id: inserted.id, position: index + 1 }));
        if (withTest.length) {
          const { error: questionError } = await client.from("questions").insert(withTest);
          if (questionError) throw questionError;
        }
      },
      async deleteAttempt(id) {
        await client.from("attempts").delete().eq("id", id);
      },
      async deleteTest(id) {
        await client.from("tests").delete().eq("id", id);
      }
    };
  }
...
function bindEvents() {
  ...
  document.getElementById("openAdminBtn").addEventListener("click", () => el.adminLoginOverlay.classList.remove("hidden"));
  ...
  el.adminLoginForm.addEventListener("submit", (event) => {
    ...
    if (el.adminUser.value === ADMIN_USER && el.adminPass.value === ADMIN_PASS) {
